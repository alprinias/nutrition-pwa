...
const LineChart = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      const vals = props.points.map(p => p[props.metric] ?? 0)
      if (!vals.length) return null

      const maxVal = Math.max(...vals, props.target ?? 0) * 1.1 || 1
      const minVal = 0
...
async function loadData() {
  ...
  ;(data ?? []).forEach(m => {
    ...
    if (!dayMap[key]) dayMap[key] = { date: key, calories: 0, protein: 0, carbs: 0, fat: 0, n: 1 }
